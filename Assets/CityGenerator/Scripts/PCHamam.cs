...
{
    public PCHamam(Vector3 position, float size, float height, string buildingName, Texture t)
        : base(buildingName)
    {
        texture = t;
    ...
    private void GenerateModel(Vector3 position, float size, float height)
    {
        ...
        hamamWalls.transform.localScale =
            new Vector3(size, height * 2 - 4, size) / ProceduralCityGenerator.ScaleFactor;
        ...
        hamamWalls2.transform.localScale =
            new Vector3(size * 0.5f, height * 2, size * 0.5f) / ProceduralCityGenerator.ScaleFactor;
        hamamWalls2.transform.localPosition =
            new Vector3(-size * 0.25f, 0, size * 0.25f) / ProceduralCityGenerator.ScaleFactor;
        ...
        houseWallRenderer.material.SetTextureOffset(
            "_MainTex",
            new Vector2(UnityEngine.Random.value, Random.value)
        );
